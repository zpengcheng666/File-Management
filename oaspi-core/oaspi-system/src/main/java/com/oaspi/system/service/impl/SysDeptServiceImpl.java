...
import com.oaspi.common.core.domain.TreeSelect;
...
import com.oaspi.system.mapper.SysDeptMapper;
...
public class SysDeptServiceImpl implements ISysDeptService
{
     ...
     */
    @Override
    public List<SysDept> selDeptList(SysDept dept)
    {
        return deptMapper.selDeptList(dept);
    }
     ...
     */
    @Override
    public List<SysDept> selDept(SysDept dept)
    {
        return deptMapper.selDept(dept);
    }

    @Override
    @DataScope(deptAlias = "d")
    public List<SysDept> selectCorpList(SysDept dept)
    {
        return deptMapper.selectCorpList(dept);
    }
...
//        return deptMapper.selectDeptListByRoleId(roleId, role.isDeptCheckStrictly());
    ...
    public int insertDept(SysDept dept)
    {
        ...
        updateDeptTenant(dept);
        dept.setAncestors(info.getAncestors() + "," + dept.getParentId());
        return deptMapper.insertDept(dept);
    }

    @Override
    public int insertCorp(SysDept dept) {
        dept.setParentId(0L);
        dept.setAncestors("0");
        updateDeptTenant(dept);
        return deptMapper.insertDept(dept);
    }
    private void updateDeptTenant(SysDept dept){
        if (!SysUser.isAdmin(SecurityUtils.getUserId())){
            if(dept.getTenantId() == null || dept.getTenantId() == 0){
                dept.setTenantId(SecurityUtils.getLoginUser().getTenantId());
                if(dept.getTenantId() == null || dept.getTenantId() == 0){
                    throw new ServiceException("租户ID为空，无法修改公司信息");
                }
            }
        }
    }
     ...
     */
    @Override
    public AjaxResult bindDept(SysDept dept) {
        //插入时间
        dept.setUpdateTime(DateUtils.getNowDate());
        dept.setUpdateBy(String.valueOf(SecurityUtils.getUserId()));
        //修改绑定关系
        deptMapper.bindDept(dept);
